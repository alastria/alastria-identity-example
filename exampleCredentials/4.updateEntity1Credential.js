...
const updateCredentialStatus = transactionFactory.credentialRegistry.updateCredentialStatus(web3, credentialHash.psmhash, configData.updateIssuerCredentialTo)
...
async function main() {
  const updateCredStat = await entity1Identity.getKnownTransaction(updateCredentialStatus)
  ...
  web3.eth.sendSignedTransaction(updateCredStat)
    .then(() => {
			const issuerCredentialTransaction = transactionFactory.credentialRegistry.getIssuerCredentialStatus(web3, configData.didEntity1, credentialHash.psmhash)
