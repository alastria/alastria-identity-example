...
const verifyJWT = tokensFactory.tokens.verifyJWT(signedJWT, '04'+config.adminPubk.substr(2))
...
const psmHashSubject = tokensFactory.tokens.PSMHash(web3, signedJWT, config.didSubject1);
console.log("\tThe PSMHash is:", psmHashSubject);

const psmHashReciever = tokensFactory.tokens.PSMHash(web3, signedJWT, config.didEntity2);
console.log("\tThe PSMHashReciever is:", psmHashReciever);

console.log("\n---- Create AIC ----")
// create AIC

const txCreateAlastriaID = transactionFactory.identityManager.createAlastriaIdentity(web3, config.adminPubk)
const txCreateAlastriaIDSigned = tokensFactory.tokens.signJWT(txCreateAlastriaID, adminPrivateKey)

// Only the createAlastriaID transaction must be signed inside of AIC object
const aic = tokensFactory.tokens.createAIC(txCreateAlastriaIDSigned,alastriaToken,config.adminPubk);
console.log("\tAIC:", aic);
