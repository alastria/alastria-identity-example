...
const createPresentation = tokensFactory.tokens.createPresentation(presentationData.credentials[0].header.kid,
...
const signedJWTPresentation = tokensFactory.tokens.signJWT(createPresentation, subject1PrivateKey)
...
const addPresentationTransaction = transactionFactory.presentationRegistry.addSubjectPresentation(web3, subjectPresentationHash, uri)

async function main() {
  const subject1PresentationSigned = await subject1Identity.getKnownTransaction(addPresentationTransaction)
