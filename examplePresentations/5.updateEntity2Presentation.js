...
async function main() {
  const updateReceivP = await entity2Identity.getKnownTransaction(updateEntity2Presentation)
  ...
  web3.eth.sendSignedTransaction(updateReceivP)
    .then(() => {
      const presentationStatus = transactionFactory.presentationRegistry.getReceiverPresentationStatus(web3, configData.didEntity2, presentationHash.psmhash)
