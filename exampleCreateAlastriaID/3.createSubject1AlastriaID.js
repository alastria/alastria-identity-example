...
function prepareAlastriaId() {
	const preparedId = transactionFactory.identityManager.prepareAlastriaID(web3, subject1Keystore.address)
	return preparedId
}

function createAlastriaId() {
	const txCreateAlastriaID = transactionFactory.identityManager.createAlastriaIdentity(web3, configData.subject1Pubk.substr(2))
...
async function main() {
	...
	const signedCreateTransaction = await subject1Identity.getKnownTransaction(createResult)
	...
	const subjectSignedAT = tokensFactory.tokens.signJWT(signedAT, subject1PrivateKey)
	const aic = tokensFactory.tokens.createAIC(signedCreateTransaction,subjectSignedAT,configData.subject1Pubk.substr(2));
