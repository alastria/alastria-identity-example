...
const keyDataSubject2 = fs.readFileSync('../keystores/subject2-643266eb3105f4bf8b4f4fec50886e453f0da9ad.json')
...
const subject2Identity = new UserIdentity(web3, `0x${subject2Keystore.address}`, subject2PrivateKey)


console.log('\n ------ Example of prepare Alastria ID, addKey and createAlastrisID necessary to have an Alastria ID ------ \n')

function preparedAlastriaId() {
	const preparedId = transactionFactory.identityManager.prepareAlastriaID(web3, subject2Keystore.address)
	return preparedId
}

function createAlastriaId() {
	const txCreateAlastriaID = transactionFactory.identityManager.createAlastriaIdentity(web3, configData.subject2Pubk.substr(2))
...
async function main() {
	...
	const at = tokensFactory.tokens.createAlastriaToken(config.didEntity1, config.providerURL, config.callbackURL, config.networkId, config.tokenExpTime, config.tokenActivationDate, config.jsonTokenId)
	...
	const signedCreateTransaction = await subject2Identity.getKnownTransaction(createResult)
	...
	const subjectSignedAT = tokensFactory.tokens.signJWT(signedAT, subject2PrivateKey)
	const aic = tokensFactory.tokens.createAIC(signedCreateTransaction,subjectSignedAT,configData.subject2Pubk);
	...
	const signedPreparedTransaction = await entity1Identity.getKnownTransaction(prepareResult)
