...
const entity2Identity = new UserIdentity(web3, `0x${entity2Keystore.address}`, entity2PrivateKey)
...
function preparedAlastriaId() {
	const preparedId = transactionFactory.identityManager.prepareAlastriaID(web3, entity2Identity.address)
	return preparedId
}

function createAlastriaId() {
	const txCreateAlastriaID = transactionFactory.identityManager.createAlastriaIdentity(web3, configData.entity2Pubk.substr(2))
...
async function main() {
	...
	const signedCreateTransaction = await entity2Identity.getKnownTransaction(createResult)
	...
	const entitySignedAT = tokensFactory.tokens.signJWT(signedAT, entity2PrivateKey)
	const aic = tokensFactory.tokens.createAIC(signedCreateTransaction,entitySignedAT, configData.entity2Pubk.substr(2));
